...
N_RUNS   = 5
MAX_ITER = 5
QUERIES  = ["q3", "q4", "q5"]  # closures only on non-trivial queries
...
def main(base_llm, queries_to_run: list[str]):
    ...
    for qid in queries_to_run:
        ...
        pipe_m = ClosurePipeline([M], n_break=2, max_iter=8)
        last = []
        agg  = _aggregate(
            lambda p=pipe_m: run_llmscan_closure(
                pattern, env, gamma, tracker, return_var, p),
            gt, N_RUNS, tracker, last)
        report.add("LLMScan\_Motiv", agg, actual=last[0], expected=gt)

        # ── LLMScan_Telephone ─────────────────────────────────────────────────
        T = TelephoneClosure(n_names=5)
        pipe_t = ClosurePipeline([T], n_break=2, max_iter=8)
        last = []
        agg  = _aggregate(
            lambda p=pipe_t: run_llmscan_closure(
                pattern, env, gamma, tracker, return_var, p),
            gt, N_RUNS, tracker, last)
        report.add("LLMScan\_Tel", agg, actual=last[0], expected=gt)
